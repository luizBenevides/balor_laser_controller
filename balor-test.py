...
data = job.serialize()
# Ensure packet alignment
packet_size = 3072 # Default for BJJCZ_LMCV4_FIBER_M
if len(data) % packet_size:
    padding_needed = packet_size - (len(data) % packet_size)
    # Each command is 12 bytes, so we add NOPs (OpWait or similar)
    # Actually, BJJCZ padding is usually just zeros or specific NOP commands.
    # In Balor, 0x02 0x80 ... is a NOP.
    nop = bytearray([0x02, 0x80] + [0]*10)
    data += nop * (padding_needed // 12)
