...
class BJJCZ_LMCV4_FIBER_M(Machine.Machine):
    ...
    def connect_device(self, index=0):
        print(f"[DEBUG] Attempting to connect to BJJCZ device index {index}...")
        import libusb_package
        backend = libusb_package.get_libusb1_backend()
        print(f"[DEBUG] Using libusb backend: {backend}")
        
        devices=list(usb.core.find(find_all=True, idVendor=0x9588, idProduct=0x9899, backend=backend))
        print(f"[DEBUG] Found {len(devices)} device(s) with VID=0x9588, PID=0x9899")
        
        if not devices:
            raise Exception("No BJJCZ devices found. Check cable and power.")
            
        try:
            device = devices[index]
        except IndexError:
            raise Exception(f"Device index {index} not found. Total devices: {len(devices)}")

        try:
            self.manufacturer = usb.util.get_string(device, device.iManufacturer)
            self.product = usb.util.get_string(device, device.iProduct)
            print(f"[DEBUG] Device info: {self.manufacturer} - {self.product}")
        except Exception as e:
            print(f"[DEBUG] Warning: Could not read device strings: {e}")

        print("[DEBUG] Setting configuration...")
        try:
            device.set_configuration() # It only has one.
            print("[DEBUG] Configuration set successfully.")
        except Exception as e:
            print(f"[DEBUG] set_configuration failed (expected on some Windows setups): {e}")
        
        print("[DEBUG] Checking kernel driver...")
        try:
            if device.is_kernel_driver_active(0):
                print("[DEBUG] Detaching kernel driver...")
                device.detach_kernel_driver(0)
        except Exception as e:
            print(f"[DEBUG] Kernel driver check/detach failed (expected on Windows): {e}")

        print("[DEBUG] Claiming interface 0...")
        try:
            usb.util.claim_interface(device, 0)
            print("[DEBUG] Interface 0 claimed successfully.")
        except Exception as e:
            print(f"[DEBUG] CRITICAL: Failed to claim interface 0: {e}")
            raise e
