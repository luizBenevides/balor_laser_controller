...
class Sender:
    ...
    def open(self, machine_index=0, mock=False, **kwargs):
        ...
        time.sleep(0.2)  # Wait for the machine to be ready after connection
        self._init_machine(**kwargs)
        return True

    def close(self):
        # Only try to abort if we actually have a connection
        if self._usb_connection is not None:
            try:
                self.abort()
            except:
    ...
    def _send_command(self, *args, **kwargs):
        if self._usb_connection is None:
            return 0, 0 # Silent fail or handle better
    ...
    def _init_machine(self,
        ...
        self._log_debug("Initializing machine...")
        try:
            self._log_debug("Fetching serial number...")
            self.serial_number = self.raw_get_serial_no()
            self._log_debug(f"Serial Number: {self.serial_number}")
            
            self._log_debug("Fetching version...")
            self.version = self.raw_get_version()
            self._log_debug(f"Version: {self.version}")
            
            self.raw_get_st_mo_ap()
            
            # Unknown function
            self.raw_reset()
            self._log_debug("Machine reset sent.")
        except Exception as e:
            self._log_debug(f"Error during early init: {e}")
            raise e

        # Load in-machine correction table
        cor_table = None
        if cor_file is not None:
            self._log_debug(f"Loading correction file: {cor_file}")
            cor_table = self._read_correction_file(cor_file)
        
        self._log_debug("Sending correction table...")
        self._send_correction_table(cor_table)

        self._log_debug("Configuring laser parameters...")
        ...
        self.raw_enable_z()
        self._log_debug("Machine initialization complete.")
...
class UsbConnection:
    ...
    def open(self):
        self._log_debug(f"UsbConnection: Attempting to open device index {self.machine_index}...")
        import libusb_package
        backend = libusb_package.get_libusb1_backend()
        self._log_debug(f"Using libusb backend: {backend}")
        
        devices=list(usb.core.find(find_all=True, idVendor=0x9588, idProduct=0x9899, backend=backend))
        self._log_debug(f"Found {len(devices)} device(s) with VID=0x9588, PID=0x9899")
        
        if len(devices) == 0:
            raise BalorMachineException("No compatible engraver machine was found. Check cable/power.")

        try:
            device = devices[self.machine_index]
            self.device = device # Set this EARLY so close() can dispose it if we fail later
            self._log_debug(f"Selected device at index {self.machine_index}")
        except IndexError:
            raise BalorMachineException(f"Invalid machine index {self.machine_index}. Found {len(devices)} devices.")

        # if the permissions are wrong, these will throw usb.core.USBError
        self._log_debug("Setting configuration...")
        try:
            device.set_configuration()
            self._log_debug("Configuration set successfully.")
        except Exception as e:
            self._log_debug(f"set_configuration failed (expected on some Windows setups): {e}")
            
        self._log_debug("Claiming interface 0...")
        try:
            usb.util.claim_interface(device, 0)
            self._log_debug("Interface 0 claimed successfully.")
        except Exception as e:
            self._log_debug(f"CRITICAL: Failed to claim interface 0: {e}")
            raise e

        self._log_debug("Connected.")

    def close(self):
        if self.device:
            try:
                # Try to release interface if possible
                try:
                    usb.util.release_interface(self.device, 0)
                except:
                    pass
                usb.util.dispose_resources(self.device)
                self._log_debug("USB resources disposed.")
            except Exception as e:
                self._log_debug(f"Error disposing resources: {e}")
        self.device = None
        self.status = None
    ...
    def send_correction_entry(self, correction):
        ...
        if not self.device: raise BalorCommunicationException("No usb connection.")
        # This is really a command and should just be issued without reading.
        query = bytearray([0x10] + [0] * 11)
        query[2:2 + 5] = correction
        if self.device.write(self.ep_homi, query, 1000) != 12:
    ...
    def send_command(self, code, *parameters, read=True):
        ...
        self._log_debug("---> " + str(query))
        if read:
            try:
                response = self.device.read(self.ep_himo, 8, 1000)
                if len(response) != 8:
                ...
                self._log_debug(f"USB Read Error: {e}")
                raise e
        else:
            return 0, 0
    ...
    def send_list_chunk(self, data):
        ...
        sent = self.device.write(self.ep_homi, data, 1000)
        if sent != len(data):
            raise BalorCommunicationException("Could not send list chunk")
        self._log_debug("---> " + str(data))
...
class MockConnection:
    ...
    def send_command(self, code, *parameters, read=True):
        """Send a command to the machine and return the response.
           Updates the host condition register as a side effect."""
        self._log_debug("---> " + str(code) + " " + str(parameters))
        time.sleep(0.005)
