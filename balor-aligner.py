...
parser.add_argument('--corfile',
    help="Provide a correction file for the machine.")
...
#        self.sender = sender
#        self.sender.execute(self.geometry, loop_count=inf)
...
class Aligner:
    ...
    def thread_loop(self):
        while self.run_loop:
            if not self.sender.is_busy() or self.reload_geometry:
                if self.lasering: 
                    print ("Loading new lasering geometry!")
                    self.sender.execute(self.mark_geometry, 
                                    loop_count=1,
                                    callback_finished=self.done_lasering)
                    print ("Ended laser marking execution")
                    self.lasering = False
                else:
                    self.sender.execute(self.geometry, 
                                    loop_count=2)
                self.reload_geometry = False
            time.sleep(0.0025)
    ...
    def laser_mark(self):
        ...
        # Make a mark
        self.mark_geometry = mark_geometry
        self.lasering = True
        self.reload_geometry = True
        time.sleep(0.1)

        #while not self.sender.is_busy():
        #    time.sleep(0.01)

        #while self.sender.is_busy():
        #    time.sleep(0.01)
    ...
    def square_marking_geometry(self):
        ...
        job = balor.command_list.CommandList(self.args.machine)
        ...
        job.append(balor.command_list.OpTravel(nx, ny))
        job.laser_control(True)
        for vx,vy in square:
            job.append(balor.command_list.OpCut(vx,vy,0x8000))
        job.laser_control(False)
        open("laserdebug",'wb').write(job.serialize())
        return job
    ...
    def draw_geometry(self, first_run=False):
        ...
        job = balor.command_list.CommandList(sender=self.sender, cal=self.cal)
        job.set_travel_speed(self.args.travel_speed)
        cur_point = self.i_which_point.get()

        aox,aoy = self.alignment_offset
        ox,oy = self.cal.interpolate(aox,aoy)
        job.append(balor.command_list.OpTravel(ox, oy))
        ...
        for which_point in range(3):
            ...
            #job.append(balor.command_list.OpTravel(0x8000, 0x8000))
            job.append(balor.command_list.OpTravel(cx, cy))
            for _ in range(13 if which_point == cur_point else 3):
                for x,y in square[which_point:which_point+4]:
                    job.append(balor.command_list.OpTravel(x,y))
            job.append(balor.command_list.OpTravel(ox, oy))
        ...
        if self.i_light_origin.get():
            ...
            for _ in range(2):
                ...
                job.append(balor.command_list.OpTravel(xaxp,xayp))
                job.append(balor.command_list.OpTravel(xaxp2,xayp2))
                job.append(balor.command_list.OpTravel(xaxp3,xayp3))
                job.append(balor.command_list.OpTravel(xaxp,xayp))

                job.append(balor.command_list.OpTravel(xaxn,xayn))
                job.append(balor.command_list.OpTravel(ox, oy))
                

                job.append(balor.command_list.OpTravel(yaxp,yayp))
                job.append(balor.command_list.OpTravel(yaxp2,yayp2))
                job.append(balor.command_list.OpTravel(yaxp,yayp))
                job.append(balor.command_list.OpTravel(yaxp3,yayp3))
                job.append(balor.command_list.OpTravel(yaxp,yayp))

                job.append(balor.command_list.OpTravel(yaxn,yayn))
        job.append(balor.command_list.OpTravel(ox, oy))
    ...
    def update(self):
        ...
        report = self.sender.get_condition()
        status = 'Register 0x07: 0x%04X'%report
        self.machine_status7.config(text=status)
        try:
            dg = self.dirty_geometry
            self.geometry = self.draw_geometry(self.dirty_geometry)
            if not self.lasering and dg:
                print ("flagging geometry for reload")
                self.reload_geometry = True
        except ValueError:
            pass
...
machine.open(machine_index=args.index, cor_file=args.corfile)
try:
