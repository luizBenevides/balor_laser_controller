...
parser.add_argument('operation', 
        help="choose operational mode (in lighting mode, a bounding box will be drawn with crosshairs at 0,0.)", default="light", choices=["mark", "light"])

parser.add_argument('-f', '--file', 
        help="gcode / ngc file to load (default stdin).", default=None)
...
def arc_points(ox, oy, r, start_a, finish_a, segs_per_rad=10):
    seg_count = int(round(abs(start_a-finish_a)*segs_per_rad))
    if seg_count < 1:
        seg_count = 1
    step = (finish_a - start_a)/seg_count
    rv = []
    for n in range(seg_count):
        angle = start_a + (1+n)*step
...
def render_gcode_bounding(gfile, job, cal, args):
    ...
    while i < len(gfile.lines):
        line = gfile.lines[i]
        if line.comment: print ("#", line.comment, file=sys.stderr)
        ...
        if code == 'G':
            if index == 0:
                ...
                print ("Error: Probably using the wrong units. (G20, should be G21)",
                        file=sys.stderr)
                sys.exit(-20)
            elif index == 21:
                ...
                # arc
                print ("Error: arc movement not supported")
                sys.exit(-1)
            else:
                print ("Error: unknown gcode: %s%d"%(code,index), params, file=sys.stderr)
                sys.exit(-1)
        elif code == 'M':
            if index == 0:
                if not repetition: break # program stop
            elif index in [3,4]:
                pass # Ignore spindle
        else:# code == 'F':
    ...
    while i < len(gfile.lines):
        ...
        if code == 'G':
            if index == 0:
                ...
                nx = params['X'] if 'X' in params else x
                ny = params['Y'] if 'Y' in params else y
                gi = params['I'] if 'I' in params else 0
                gj = params['J'] if 'J' in params else 0
                r = (gi**2+gj**2)**0.5
                ox = x+gi; oy = y+gj
                a0 = math.atan2(y-oy, x-ox)
                a1 = math.atan2(ny-oy, nx-ox)
                if index == 2: #clockwise
                    start_a = min(a0,a1)
                    finish_a = max(a0,a1)
                else:
                    #anticlockwise
                    start_a = max(a0,a1)
                    finish_a = min(a0,a1)
                for (x,y) in arc_points(ox, oy, r, start_a, finish_a):
                    try:
                        job.append(balor.MSBF.OpCut(*cal.interpolate(x,y)))
                    except ValueError:
                        print ("Overflow arc cut for", x, y)
        ...
        if i == len(gfile.lines) and repetition:
            repetition -= 1
            print ("Repetition %d/%d"%(args.repetition-repetition, 
                args.repetition), file=sys.stderr)
            i = first_step
